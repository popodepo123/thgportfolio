...
class ContributionService {
  ...
  static Future<SymbolMatch?> searchSymbol(String repoUrl, String symbol, {String branch = 'main', String? currentFilePath}) async {
    if (!_recursiveTreeCache.containsKey(repoUrl)) {
      ...
      for (int page = 1; page <= 5; page++) {
        final apiUrl = 'https://gitlab.com/api/v4/projects/$encodedPath/repository/tree?recursive=true&per_page=100&page=$page&ref=$branch';
        try {
          final response = await http.get(Uri.parse(wrapUrl(apiUrl))).timeout(const Duration(seconds: 10));
          if (response.statusCode == 200) {
            ...
            return searchSymbol(repoUrl, symbol, branch: 'master', currentFilePath: currentFilePath);
          } else { break; }
        } catch (e) { break; }
      }
    ...
    for (var node in tree) {
      if (node['type'] != 'blob' || !(node['path'] as String).endsWith('.dart')) continue;
      final String path = node['path'] as String;
      final String fileName = path.split('/').last.toLowerCase();
      int score = 0;
      if (fileName == targetFile) score += 100;
      final components = ['notifier', 'state', 'provider', 'repository', 'entity', 'model', 'view', 'controller'];
      for (var comp in components) {
        if (fuzzySymbol.contains(comp) && fileName.contains(comp)) {
          score += 40;
          if (fuzzySymbol.endsWith(comp) && fileName.contains(comp)) score += 20;
        }
      }
      if (currentDir != null && path.startsWith(currentDir)) score += 30;
      if (fileName.replaceAll('_', '').contains(fuzzySymbol.replaceAll('_', ''))) score += 10;
    ...
    debugPrint('JTD: Verifying top ${topCandidates.length} candidates for definition...');

    for (var candidate in topCandidates) {
      final content = await fetchRawFile(repoUrl, filePath: candidate.path, branch: branch);
      if (content != null) {
        ...
        for (int i = 0; i < lines.length; i++) {
          ...
          for (var p in patterns) {
            if (line.contains(p)) {
              debugPrint('JTD: CONFIRMED in ${candidate.path} at line ${i + 1}');
