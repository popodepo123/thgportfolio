...
class _ContributionHeatmapState extends State<ContributionHeatmap> {
  ...
  Widget _buildGridContent() {
    if (_isLoading) {
      return const HeatmapLoadingIndicator(
        key: ValueKey('loading'),
        showTitleAndLegend: false,
      );
    }
      ...
      return const SizedBox(
        ...
        child: Center(
          child: Text(
            ...
            style: TextStyle(color: gruberQuartz, fontSize: 12),
          ),
        ),
      );
    }
    ...
    return LayoutBuilder(
      ...
      builder: (context, constraints) {
        ...
        final double squareSize =
            (availableWidth - (weeksToShow * spacing * 2)) / weeksToShow;
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        const Text(
          'Less ',
          style: TextStyle(color: gruberQuartz, fontSize: 10),
        ),
